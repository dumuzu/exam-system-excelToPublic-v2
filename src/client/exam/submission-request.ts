...
function localizedFailure(locale: StudentDisplayLocale, ja: string, zh: string, en: string): string {
  if (locale === "legacy_bilingual") return `${ja} / ${en}`;
  return { ja, zh, en }[locale];
}

export function describeSubmissionFailure(error: unknown, locale: StudentDisplayLocale = "legacy_bilingual"): { dialog: string; status: string } {

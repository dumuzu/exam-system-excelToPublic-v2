export type StudentDisplayLocale = "legacy_bilingual" | "ja" | "zh" | "en";

export interface StudentLocalizedText {
  ja: string;
  zh: string;
  en: string;
}

export function formatStudentText(copy: StudentLocalizedText, locale: StudentDisplayLocale): string {
  return locale === "legacy_bilingual" ? `${copy.ja} / ${copy.en}` : copy[locale];
}

export function studentLanguageTag(locale: StudentDisplayLocale): string {
  if (locale === "zh") return "zh-CN";
  if (locale === "en") return "en";
  return "ja";
}

export function isStudentDisplayLocale(value: unknown): value is StudentDisplayLocale {
  return value === "legacy_bilingual" || value === "ja" || value === "zh" || value === "en";
}

export function resolveStudentDisplayLocale(...candidates: unknown[]): StudentDisplayLocale {
...
} satisfies Record<string, StudentLocalizedText>;

type ShellCopyKey = keyof typeof shellCopy;

const shellBindings: ReadonlyArray<readonly [selector: string, key: ShellCopyKey]> = [
...
const legacyChildren = new WeakMap<Element, Node[]>();

function cloneChildren(element: Element): Node[] {
  return Array.from(element.childNodes, (node) => node.cloneNode(true));
}

export function applyStudentShellLocale(documentRef: Document, locale: StudentDisplayLocale): void {
  ...
  for (const [selector, key] of shellBindings) {
    ...
    if (!element) continue;
    if (!legacyChildren.has(element)) legacyChildren.set(element, cloneChildren(element));
  ...
  const shortAnswer = documentRef.querySelector<HTMLTextAreaElement>("#manual-short-answer");
  if (shortAnswer) {
  ...
  const attributes: ReadonlyArray<readonly [string, string, StudentLocalizedText]> = [

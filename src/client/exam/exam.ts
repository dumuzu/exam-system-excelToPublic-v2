...
import {
  applyStudentShellLocale,
  formatStudentText,
  isStudentDisplayLocale,
  resolveStudentDisplayLocale,
  type StudentDisplayLocale,
  type StudentLocalizedText,
} from "./student-localization.js";
...
const navigationGuard: any = createAssessmentNavigationGuard({
  ...
  onNavigationBlocked: () => {
    document.querySelector("#submission-status").textContent = t(
      "戻る操作を無効にしました。答案はこのページに保持されています。",
      "已阻止返回操作，答卷仍保留在此页面。",
      "Back navigation was blocked. Your answer sheet remains on this page.",
    );
  },
});
...
function studentDisplayLocale(): StudentDisplayLocale {
  const locale: unknown = studentState.studentLocale;
  return isStudentDisplayLocale(locale) ? locale : "legacy_bilingual";
}
function studentText(copy: StudentLocalizedText): string {
  return formatStudentText(copy, studentDisplayLocale());
}
function t(ja: string, zh: string, en: string): string {
  return studentText({ ja, zh, en });
}
function studentDateLocale(): string {
  return studentDisplayLocale() === "zh" ? "zh-CN" : studentDisplayLocale() === "en" ? "en-US" : "ja-JP";
}
function setLocalizedPair(primary: Element, secondary: HTMLElement, copy: StudentLocalizedText): void {
...
function applyStudentDisplayLocale(result: unknown) {
  ...
  const locale = resolveStudentDisplayLocale(
    resultStudentDisplayLocale(result),
    studentState.studentLocale,
    datasetLocale,
  );
...
async function reportViolation(integritySignal: any, { acknowledgementSeconds = VIOLATION_ACKNOWLEDGEMENT_SECONDS, fullscreenGraceMs = null }: any = {}) {
  ...
  try {
    ...
    const confirm: any = document.querySelector("#violation-confirm");
    const countdown: any = document.querySelector("#violation-countdown");
    const returnLabel: any = t("試験に戻る", "返回考试", "Return to exam");
...
function startHeartbeat() {
  ...
  const send: any = async () => {
    ...
    try {
      ...
      if (studentState.heartbeatFailures >= 2) document.querySelector("#submission-status").textContent = t("通信が不安定です。入力内容はこの端末にも保存しています。", "网络连接不稳定，输入内容也已暂存在本设备。", "The connection is unstable. Your input is also stored on this device.");
    }
  };
...
function renderManualResponse(question: any, savedValue: any) {
  ...
  if (question.questionMode === "fill_blank") {
    ...
    for (const segment of question.segments ?? []) {
      ...
      const input: any = document.createElement("input"); input.type = "text"; input.maxLength = 500; input.value = values[segment.id] ?? ""; input.autocomplete = "off"; input.setAttribute("aria-label", `${t("空欄", "填空", "Blank")} ${segment.id}`);
...
document.querySelector("#terminal-recheck").addEventListener("click", async (event: any) => {
  ...
  } catch { document.querySelector("#terminal-status").textContent = t("確認できませんでした。先生に知らせてください。", "检查失败，请通知教师。", "Check failed. Ask your teacher."); }
...
document.querySelector("#submit-confirm").addEventListener("click", async (event: any) => {
  ...
  try {
    ...
    const copy: any = describeSubmissionFailure(error, studentDisplayLocale());

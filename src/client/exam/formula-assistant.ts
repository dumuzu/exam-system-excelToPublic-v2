...
export interface FormulaCompletionResult {
  formula?: string;
  query: string;
  start: number;
  end: number;
  items: FormulaFunctionHelp[];
}
...
const functionHelp = (name: string, syntax: string, descriptionJa: string, descriptionEn: string): FormulaFunctionHelp => ({ name, syntax, descriptionJa, descriptionEn });
...
function normalizedFormula(value: unknown): string {
  return String(value ?? "").normalize("NFKC");
}

function completionContext(value: unknown, cursor: number): CompletionContext | null {
  ...
  if (start > 1 && (!previous || !/[,(+\-*/^]/.test(previous))) return null;
  return { formula, start, end: safeCursor, query: query.toUpperCase() };
}

export function getFunctionCompletions(value: unknown, cursor = String(value ?? "").length): FormulaCompletionResult {
  const context = completionContext(value, cursor);
  if (!context) return { query: "", start: cursor, end: cursor, items: [] };
  return {
    ...context,
...
export function applyFunctionCompletion(value: unknown, cursor: number, functionName: unknown): { value: string; cursor: number } {
  const context = completionContext(value, cursor);
  const selected = HELP_BY_NAME.get(String(functionName ?? "").toUpperCase());
  if (!context || !selected) return { value: String(value ?? ""), cursor: Number(cursor) || 0 };
...
export function findActiveFunctionHelp(value: unknown, cursor = String(value ?? "").length): FormulaFunctionHelp | null {

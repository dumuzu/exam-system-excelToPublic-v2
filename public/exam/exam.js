...
function isStudentDisplayLocale(value) {
...
function studentDisplayLocale() {
    const locale = studentState.verifiedExam?.exam?.studentLocale;
    return isStudentDisplayLocale(locale) ? locale : "legacy_bilingual";
}
function applyStudentDisplayLocale(result) {
    const currentLocale = studentState.verifiedExam?.exam?.studentLocale;
...
async function reportViolation(integritySignal, { acknowledgementSeconds = VIOLATION_ACKNOWLEDGEMENT_SECONDS, fullscreenGraceMs = null } = {}) {
    ...
    try {
        ...
        const confirm = document.querySelector("#violation-confirm");
        const countdown = document.querySelector("#violation-countdown");
        const returnLabelJa = "試験に戻る";
        const returnLabelEn = "RETURN TO EXAM";
    ...
    const send = async () => {
        ...
        try {
            ...
            if (studentState.heartbeatFailures >= 2)
                document.querySelector("#submission-status").textContent = "通信が不安定です。入力内容はこの端末にも保存しています。";
        }
    };
    send();
...
function renderManualResponse(question, savedValue) {
    ...
    if (question.questionMode === "fill_blank") {
        ...
        for (const segment of question.segments ?? []) {
            ...
            input.setAttribute("aria-label", `空欄 ${segment.id}`);
...
function renderAttempt(attempt) {
    ...
    submitButton.replaceChildren(document.createTextNode(isAssignmentMode() ? "答案を提出する" : "問題を確認してください"), document.createElement("br"), Object.assign(document.createElement("small"), {
    ...
    if (!isAssignmentMode()) {
        studentState.submitUnlockTimer = setTimeout(() => {
            ...
            submitButton.replaceChildren(document.createTextNode("答案を提出する"), document.createElement("br"), Object.assign(document.createElement("small"), { textContent: "FINAL SUBMISSION" }));
...
document.querySelector("#terminal-recheck").addEventListener("click", async (event) => {
    ...
    try {
        ...
        document.querySelector("#terminal-status").textContent = "確認できませんでした。先生に知らせてください。 / Check failed. Ask your teacher.";
    }
    finally {
        button.disabled = false;
    }
...
document.querySelector("#submit-confirm").addEventListener("click", async (event) => {
    ...
    try {
        ...
        finalButton.replaceChildren(document.createTextNode("内容を確認してください"), document.createElement("br"), Object.assign(document.createElement("small"), { textContent: "REVIEW FIRST" }));
        ...
        studentState.finalSubmitConfirmTimer = setTimeout(() => {
            ...
            finalButton.replaceChildren(document.createTextNode("最終確定して提出"), document.createElement("br"), Object.assign(document.createElement("small"), { textContent: "FINAL CONFIRM & SUBMIT" }));
        }, FINAL_SUBMISSION_CONFIRMATION_MILLISECONDS);
    }
    catch (error) {
        const copy = describeSubmissionFailure(error);
...
document.querySelector("#final-submit-confirm").addEventListener("click", async (event) => {
    ...
    try {
        ...
        button.replaceChildren(document.createTextNode("最終確定して提出"), document.createElement("br"), Object.assign(document.createElement("small"), { textContent: "FINAL CONFIRM & SUBMIT" }));
        button.disabled = false;
    }
});
...
document.querySelector("#violation-confirm").addEventListener("click", async () => {
    ...
    try {
        ...
        document.querySelector("#violationMessageEn").textContent = "Fullscreen could not be restored. Ask your teacher for help.";
    }
});

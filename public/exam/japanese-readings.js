...
function appendPlain(tokens, text) {
...
export function createJapaneseReadingTokens(value) {
    ...
    while (index < text.length) {
        ...
        if (!term) {
            appendPlain(tokens, text[index] ?? "");
            index += 1;
            continue;
        }
        if (seen.has(term))
            appendPlain(tokens, term);
        else {
            tokens.push({ text: term, reading: readings.get(term) });
            seen.add(term);
        }
        index += term.length;
    }
...
export function renderJapaneseWithReadings(element, value) {
